...
#######################################################################################################################
    ...
    def get_pool_address(token_0: str, token_1: str, fee: int) -> str:
        ...
        pools = {
            ...
            'WBTC/USDC 5bps':'0x9a772018fb77fcd2d25657e5c547baff3fd7d16',
    ...
    def get_tick_nets(self):
        """
        Retrieves tick data and corresponding liquidity nets from the Uniswap pool.

        Returns:
            tick_data (list): List of tick positions.
            liquidity_nets (list): List of cumulative liquidity nets.
            """
        # Get the current tick and tick spacing
        current_tick = self.data['tick'].iloc[-1]  # Assuming last tick is the current
        tick_spacing = self.tick_spacing
        scaled_tick = (current_tick // tick_spacing) * tick_spacing

        # Initialize tick data and liquidity nets arrays
        tick_data = []
        liquidity_nets = []

        # Simulate the range of ticks (-150 to +150 relative to scaled tick)
        for dt in range(-150, 151):  # Including 150
            tick_position = scaled_tick + dt * tick_spacing

            # Fetch liquidityNet from the data (mocked as cumulative sum here)
            if len(tick_data) == 0:
                # Initial conditions
                liquidity_net = 250220217232024050  # Example value from the Solidity contract
            else:
                liquidity_net = liquidity_nets[-1] + self.data['liquidity_total'].get(tick_position, 0)

            # Append to results
            tick_data.append(tick_position)
            liquidity_nets.append(liquidity_net)

        return tick_data, liquidity_nets
